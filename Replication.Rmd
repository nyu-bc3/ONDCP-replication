...
nat_data        = read_dta("./MORTALITY_NFLIS DATA_YEAR_MONTH.dta")
state_data_yr   = read_dta("./MORTALITY_NFLIS DATA_STATE_YEAR.dta")
state_data_mnth = read_dta("./MORTALITY_NFLIS DATA_STATE_YEAR_MONTH.dta")
...
division_data_mnth <- state_data_mnth %>%
  dplyr::select( # select required variables
    ...
    death_non_synth_n_1, death_non_synth_n_9,
  ...
  summarise( # sum all variables across states within a division
    ...
    death_non_synth_n_1 = sum(death_non_synth_n_1),
    death_non_synth_n_9 = sum(death_non_synth_n_9),
...
division_data_mnth <- division_data_mnth %>%
  mutate(
    ...
    # imputation sens. analysis rates
    death_non_synth_n_1_rate = (death_non_synth_n_1 / population) * 100000,
    death_non_synth_n_9_rate = (death_non_synth_n_9 / population) * 100000,
...
# plot rates
plot = ggplot(nat_data, aes(x = YearMonth)) +
...
# save plot
ggsave("../ONDCP/fig1.pdf", plot = plot, width = 7, height = 5, units = "in")
```
...
plot = ggplot(division_data_mnth, aes(x = YearMonth)) +
...
# save plot
ggsave("../ONDCP/fig2.pdf", plot = plot, width = 7, height = 5, units = "in")
plot
```
...
# replicates supplementary table 4
# displays overall and division-wise coefficients
# for two outcomes: imputations as 1 and 9 for suppressed counts

# init. outcome vector
outcomes = c("1-Imputation", "9-Imputation")
# init. agg. model list
model_results = list()
...
# Overall National Model
# pop-weighted TWFE OLS on national-level data (fixest)
# dependent variable imputed as ones
# -----------------------------------------------------

model_list[["Overall"]] <- feols(
  death_non_synth_n_1_rate ~ prop_fentanyl_illicit | # formula
  residence_state + YearMonth,                       # state and time-wise TWFE
  data    = state_data_mnth,                         # use national dataset
  weights = ~population,                             # population-weighted
  cluster = ~residence_state + YearMonth             # two-way clustered SEs
...
for (pd in unique(state_data_mnth$time)) {
  ...
  # Run pop-weighted TWFE OLS model
  model <- feols( # include the time factor
    death_non_synth_n_1_rate ~ prop_fentanyl_illicit * factor(refactor) |
    residence_state + YearMonth,           # state and time-wise TWFE
    data    = state_data_mnth,             # dataset
    weights = ~population,                 # population-weighted
    cluster = ~residence_state + YearMonth # two-way clustered SEs
...
# Overall National Model
# pop-weighted TWFE OLS on national-level data (fixest)
# dependent variable imputed as nines
# -----------------------------------------------------

model_list[["Overall"]] <- feols(
  death_non_synth_n_9_rate ~ prop_fentanyl_illicit | # formula
  residence_state + YearMonth,                       # state and time-wise TWFE
  data    = state_data_mnth,                         # use national dataset
  weights = ~population,                             # population-weighted
  cluster = ~residence_state + YearMonth             # two-way clustered SEs
...
for (pd in unique(state_data_mnth$time)) {
  ...
  # Run pop-weighted TWFE OLS model
  model <- feols( # include the time factor
    death_non_synth_n_9_rate ~ prop_fentanyl_illicit * factor(refactor) |
    residence_state + YearMonth,           # state and time-wise TWFE
    data    = state_data_mnth,             # dataset
    weights = ~population,                 # population-weighted
    cluster = ~residence_state + YearMonth # two-way clustered SEs
...
colnames(df_coef) = c(
  "Model",
  "Suppressed Values as 1", 
  "Suppressed Values as 9"
)
...
# replicates supplementary table 5
# considers yearly time periods
...
# -----------------------------------------------------
# Individual Year Models (incl. in reg. formula)
# pop-weighted TWFE OLS on national-level data (fixest)
# -----------------------------------------------------

# iterate thru time periods
for (pd in unique(state_data_mnth$Year)) {

  # refactoring for easier inferences
  state_data_mnth$refactor = factor(state_data_mnth$Year)
  state_data_mnth$refactor = relevel(state_data_mnth$refactor, pd)
